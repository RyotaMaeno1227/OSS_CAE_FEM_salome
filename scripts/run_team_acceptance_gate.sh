...
REQUIRE_PLAN_DECLARE="${TEAM_ACCEPTANCE_REQUIRE_PLAN_DECLARE:-1}"
...
case "${REQUIRE_PLAN_DECLARE}" in
  0)
    ;;
  1)
    AUDIT_ARGS+=(--require-plan-declare)
    ;;
  *)
    echo "ERROR: invalid TEAM_ACCEPTANCE_REQUIRE_PLAN_DECLARE: ${REQUIRE_PLAN_DECLARE} (use: 0|1)" >&2
    exit 2
    ;;
esac
