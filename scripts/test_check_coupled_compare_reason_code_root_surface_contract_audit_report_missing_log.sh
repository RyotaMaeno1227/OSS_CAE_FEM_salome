...
if python3 ./scripts/check_coupled_compare_reason_code_root_surface_contract_audit_report.py \
    "${REPORT_LOG}" \
    >/tmp/test_check_coupled_compare_reason_code_root_surface_contract_audit_report_missing_log.log 2>&1; then
    echo "FAIL: validator unexpectedly accepted missing focused bundle log" >&2
    exit 1
fi

grep -q '^FAIL: root_surface_contract_audit_log_path missing:' \
    /tmp/test_check_coupled_compare_reason_code_root_surface_contract_audit_report_missing_log.log

echo "PASS: coupled_compare reason-code root surface contract audit report validator rejects missing logs"
