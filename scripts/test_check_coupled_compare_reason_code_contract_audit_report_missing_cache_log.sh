...
if python3 ./scripts/check_coupled_compare_reason_code_contract_audit_report.py \
    "${REPORT_LOG}" \
    >/tmp/test_check_coupled_compare_reason_code_contract_audit_report_missing_cache_log.log 2>&1; then
    echo "FAIL: validator unexpectedly accepted missing cache log" >&2
    exit 1
fi

grep -q '^FAIL: contract_audit_cache_log missing:' \
    /tmp/test_check_coupled_compare_reason_code_contract_audit_report_missing_cache_log.log

echo "PASS: coupled_compare reason-code contract audit report validator rejects missing cache logs"
