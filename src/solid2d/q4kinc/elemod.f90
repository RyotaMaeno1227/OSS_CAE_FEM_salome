...
module cfmc
   ...
   integer,parameter :: nncm=2
   integer,parameter :: nmnd=4

   integer,parameter :: nint=4

   real(kind=DP_REAL_KIND) dndxl (ndim ,nnde ,nint )
   real(kind=DP_REAL_KIND) dndxlm(ndim ,nmnd ,nint )
...
contains
  ...
  &      =reshape( (/ 1,1, 2,1, 2,2, 1,2 /),(/ndim,nint/) )
      ...
      do intg=1,nint
         ...
         dndxlm(1,1,intg)=-                    xl(1)&
        &                * (-one+    xl(2)   )*xl(2)
         dndxlm(2,1,intg)=-(-one+    xl(1)**2)      &
        &                * (-one+two*xl(2)   )      *d2 
         dndxlm(1,2,intg)=-( one+two*xl(1)   )      &
        &                * (-one+    xl(2)**2)      *d2
         dndxlm(2,2,intg)=-( one+    xl(1)   )*xl(1)&
        &                *                     xl(2)
         dndxlm(1,3,intg)=-                    xl(1)&
        &                * ( one+    xl(2)   )*xl(2)
         dndxlm(2,3,intg)=-(-one+    xl(1)**2)      &
        &                * ( one+two*xl(2)   )      *d2
         dndxlm(1,4,intg)=-(-one+two*xl(1)   )      &
        &                * (-one+    xl(2)**2)      *d2
         dndxlm(2,4,intg)=-(-one+    xl(1)   )*xl(1)&
        &                *                     xl(2)
