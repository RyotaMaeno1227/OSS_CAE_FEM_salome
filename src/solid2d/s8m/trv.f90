...
!     Calculate Traction force vector
      ...
      real(kind=DP_REAL_KIND),dimension(ndim  ,nsrf  ) :: xel
      real(kind=DP_REAL_KIND),dimension(ndim         ) :: sv
      ...
      do is=1,ntrs
         ...
         do intg=1,nsint
            ...
            do i=1,nsrf
               do k=1,ndim
                  sv(k)=sv(k)+dmdxl(i,intg)*xel(k,i)
               end do
            end do

            cas=wsint(intg)*sqrt(dot_product(sv,sv))
