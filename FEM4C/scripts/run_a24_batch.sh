...
# Enforce serial make execution to avoid nested wrapper races.
export MAKEFLAGS="-j1"

lock_dir="${A24_BATCH_LOCK_DIR:-${A24_SERIAL_LOCK_DIR:-/tmp/fem4c_a24_batch.lock}}"
lock_pid_file="${lock_dir}/pid"
summary_out="${A24_BATCH_SUMMARY_OUT:-}"
summary_out_fail_reason="none"
retry_on_137="${A24_BATCH_RETRY_ON_137:-1}"
tmp_dir="$(mktemp -d)"
...
regression_test_status="skip"
regression_full_test_status="skip"
regression_attempts="0"
regression_test_attempts="0"
regression_full_test_attempts="0"
...
emit_summary() {
    ...
    if [[ -n "${summary_out}" ]]; then
        if ! printf '%s\n' "${line}" > "${summary_out}"; then
            echo "FAIL: cannot write A24 batch summary output (${summary_out})" >&2
            return 1
        fi
    fi
}
...
print_summary() {
    ...
    local line
    line="A24_BATCH_SUMMARY lock=${lock_status} retry_on_137=${retry_on_137} retry_used=${retry_used} regression=${regression_status} regression_attempts=${regression_attempts} regression_test=${regression_test_status} regression_test_attempts=${regression_test_attempts} regression_full_test=${regression_full_test_status} regression_full_test_attempts=${regression_full_test_attempts} overall=${overall} failed_step=${failed_step} failed_cmd=${failed_cmd}"
    emit_summary "${line}"
}
...
validate_summary_out() {
    ...
    if [[ ! -d "${summary_out_dir}" ]]; then
        ...
        echo "FAIL: A24 batch summary output directory does not exist (${summary_out_dir})" >&2
        return 1
    fi
    if [[ -d "${summary_out}" ]]; then
        summary_out_fail_reason="summary_out_type"
        echo "FAIL: A24 batch summary output path must be a file (${summary_out})" >&2
        return 1
    fi
    ...
    if [[ -f "${lock_pid_file}" ]]; then
        ...
        if [[ -n "${owner_pid}" ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            if mkdir "${lock_dir}" 2>/dev/null; then
                ...
                echo "INFO: recovered stale a24 batch lock (${lock_dir})"
                return 0
            fi
        fi
    else
        ...
        if mkdir "${lock_dir}" 2>/dev/null; then
            echo "$$" >"${lock_pid_file}"
            lock_status="acquired_stale_recovered"
            echo "INFO: recovered stale a24 batch lock without pid (${lock_dir})"
            return 0
        fi
    fi
...
if ! acquire_lock; then
    echo "FAIL: a24 batch lock is already held (${lock_dir})" >&2
    lock_status="held"
...
if ! [[ "${retry_on_137}" =~ ^[01]$ ]]; then
    echo "FAIL: A24_BATCH_RETRY_ON_137 must be 0 or 1 (${retry_on_137})" >&2
...
if make -C FEM4C mbd_a24_regression_test; then
    regression_test_status="pass"
    regression_test_attempts="1"
else
    regression_test_rc=$?
    regression_test_status="fail"
    regression_test_attempts="1"
    if [[ "${regression_test_rc}" -eq 137 && "${retry_on_137}" -eq 1 ]]; then
        retry_used="1"
        echo "WARN: regression_test failed with rc=137; retrying once"
        if make -C FEM4C mbd_a24_regression_test; then
            regression_test_status="pass"
            regression_test_attempts="2"
            echo "INFO: regression_test succeeded after retry"
        else
            regression_test_attempts="2"
        fi
    fi
    if [[ "${regression_test_status}" != "pass" ]]; then
        if ! print_summary "fail" "regression_test" "make_mbd_a24_regression_test"; then
            exit 1
        fi
        exit 1
    fi
...
if make -C FEM4C mbd_a24_regression_full_test; then
    regression_full_test_status="pass"
    regression_full_test_attempts="1"
else
    regression_full_test_rc=$?
    regression_full_test_status="fail"
    regression_full_test_attempts="1"
    if [[ "${regression_full_test_rc}" -eq 137 && "${retry_on_137}" -eq 1 ]]; then
        retry_used="1"
        echo "WARN: regression_full_test failed with rc=137; retrying once"
        if make -C FEM4C mbd_a24_regression_full_test; then
            regression_full_test_status="pass"
            regression_full_test_attempts="2"
            echo "INFO: regression_full_test succeeded after retry"
        else
            regression_full_test_attempts="2"
        fi
    fi
    if [[ "${regression_full_test_status}" != "pass" ]]; then
        if ! print_summary "fail" "regression_full_test" "make_mbd_a24_regression_full_test"; then
            exit 1
        fi
        exit 1
    fi
...
echo "PASS: a24 batch (serial regression/test/full-test)"
