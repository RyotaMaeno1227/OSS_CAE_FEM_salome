#!/usr/bin/env python3
"""Export pair-level FEM reduced feedback CSV from CT_237a local_oneway_rows.csv."""
...
def aggregate_status(status_values: list[str], all_valid: bool) -> str:
    ...
    if all_valid and all(
        value == "1" or value == "ok" or value == "valid" or value.startswith("ok_")
...
def main() -> int:
    if len(sys.argv) != 3:
        print(
            "Usage: python3 scripts/export_fem_local_feedback_reduced_from_local_oneway_generic.py "
    ...
    for row_index, row in enumerate(rows, start=1):
        load_step = parse_int(row["load_step"], f"row {row_index} load_step")
        ...
        grouped_rows[(load_step, pair_id)].append(
            {
                "gamma_n": gamma_n,
    ...
    with output_csv.open("w", newline="", encoding="utf-8") as handle:
        ...
        for load_step, pair_id in sorted(grouped_rows):
            records = grouped_rows[(load_step, pair_id)]
            gamma_n = statistics.fmean(float(record["gamma_n"]) for record in records)
            delta_g_eff_m = statistics.fmean(float(record["delta_g_eff_m"]) for record in records)
    ...
    print(
        "PASS export_fem_local_feedback_reduced_from_local_oneway_generic "
