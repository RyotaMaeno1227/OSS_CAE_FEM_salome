...
2. 第2章の「形状関数」だけ読む  
3. 第4章の「剛性行列の式」を確認  
4. `src/domain/fem/element/t6/t6_element.c` と対応づける

詳細な導出・演習は後回しでも構いません。最初は「式→コード」を対応させることを優先してください。

## ✅ 最小検算例（T6 形状関数）
**狙い**: 形状関数の実装が正しいか最小点で確認する。
...
手順（擬似コード）:
```
N = t6_shape_functions(xi, eta)
S = N1+N2+N3+N4+N5+N6
assert |S - 1| < 1e-10
```

判定基準:
- 1e-10 以内で 1 に一致すれば OK。
