...
!  set element data
   ...
   do j=1,nnde
      do ix=1,ndim
         do kx=1,ndim
            dndx(ix,j)=dndx(ix,j)+dxldx(kx,ix)*dndxl(kx,j,intg)
         end do
      end do
   end do
!  dndx=matmul(transpose(dxldx),dndxl(:,:,intg))
!  dndx=matmul(dxldx,dndxl(:,:,intg))

   bmat =zero
   do j=1,nnde
      jj=nddof*(j-1)
      bmat(1,jj+3)= dndx(1,j)
      bmat(2,jj+2)=-dndx(2,j)
      bmat(3,jj+2)=-dndx(1,j)
      bmat(3,jj+3)= dndx(2,j)
      bmat(4,jj+1)= dndx(1,j)
      bmat(4,jj+3)= cnxl(j,intg)
      bmat(5,jj+1)= dndx(2,j)
      bmat(5,jj+2)=-cnxl(j,intg)
   end do
