...
module cfmc
   ...
   real(kind=DP_REAL_KIND) dndxl9(ndim ,nint )
...
!     Shape function
      ...
      do intg=1,nint
         ...
         dndxl(1,1,intg)=-(-one+xl(2))*(two*xl(1)+     xl(2))*d4
         dndxl(2,1,intg)=-(-one+xl(1))*(    xl(1)+ two*xl(2))*d4
         dndxl(1,2,intg)=-(-one+xl(2))*(two*xl(1)-     xl(2))*d4
         dndxl(2,2,intg)=-( one+xl(1))*(    xl(1)- two*xl(2))*d4
         dndxl(1,3,intg)= ( one+xl(2))*(two*xl(1)+     xl(2))*d4
         dndxl(2,3,intg)= ( one+xl(1))*(    xl(1)+ two*xl(2))*d4
         dndxl(1,4,intg)= ( one+xl(2))*(two*xl(1)-     xl(2))*d4
         dndxl(2,4,intg)= (-one+xl(1))*(    xl(1)- two*xl(2))*d4

         dndxl(1,5,intg)=       xl(1) *(-one     + xl(2)    )
         dndxl(2,5,intg)=              (-one     + xl(1)**2 )*d2 
         dndxl(1,6,intg)=              ( one     - xl(2)**2 )*d2
         dndxl(2,6,intg)=-      xl(2) *( one     + xl(1)    )
         dndxl(1,7,intg)=-      xl(1) *( one     + xl(2)    )
         dndxl(2,7,intg)=              ( one     - xl(1)**2 )*d2
         dndxl(1,8,intg)=              (-one     + xl(2)**2 )*d2
         dndxl(2,8,intg)=       xl(2) *(-one     + xl(1)    )

         dndxl9(1, intg)=           xl(1)          &
        &               * (-one+    xl(2)**2)*two
         dndxl9(2, intg)= (-one+    xl(1)**2)      &
        &               *           xl(2)    *two

         cnxl(1,intg)=(one-xl(1))*(one-xl(2))*(-one-xl(1)-xl(2))*d4
         cnxl(2,intg)=(one+xl(1))*(one-xl(2))*(-one+xl(1)-xl(2))*d4
         cnxl(3,intg)=(one+xl(1))*(one+xl(2))*(-one+xl(1)+xl(2))*d4
         cnxl(4,intg)=(one-xl(1))*(one+xl(2))*(-one-xl(1)+xl(2))*d4

         cnxl(5,intg)=(one-xl(1)**2)*(one-xl(2)   )*d2
         cnxl(6,intg)=(one+xl(1)   )*(one-xl(2)**2)*d2
         cnxl(7,intg)=(one-xl(1)**2)*(one+xl(2)   )*d2
         cnxl(6,intg)=(one-xl(1)   )*(one-xl(2)**2)*d2
