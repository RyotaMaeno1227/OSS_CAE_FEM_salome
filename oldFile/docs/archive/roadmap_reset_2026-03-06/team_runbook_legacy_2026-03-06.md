...
- 参考コマンド:
  ...
  - `C_COLLECT_PREFLIGHT_LOG=latest C_COLLECT_REQUIRE_REVIEW_KEYS=1 bash scripts/run_c_team_collect_preflight_check.sh docs/team_status.md`（latest 自動解決ログに review quartet キーを必須化）
  - `run_c_team_collect_preflight_check.sh` は `collect_preflight_require_review_keys=0|1` を出力し、preflight 判定時の review quartet 必須化状態をログから直接追跡できる。
  - 上記キーは early return（`latest_not_found_*`, `latest_resolved_log_missing_*`, `explicit_log_missing`, `latest_invalid_report_default_skip`）でも必ず出力される。
  ...
  - `C_COLLECT_REQUIRE_REVIEW_KEYS=1` 併用時に latest 解決ログが review quartet 欠落なら、reason は `latest_missing_review_keys_default_skip` / `latest_missing_review_keys_strict` で区別される。
  ...
  - `check_c_team_submission_readiness.sh` は `C_REQUIRE_REVIEW_COMMANDS=1` かつ `C_COLLECT_PREFLIGHT_LOG=latest` の場合のみ、collect preflight checker に `C_COLLECT_REQUIRE_REVIEW_KEYS=1` を渡して review quartet 欠落を fail-fast する（明示ログ指定は互換維持）。
  ...
  - 同スクリプトは `collect_preflight_require_review_keys=0|1` もエントリへ自動転記し、latest probe 時に review quartet 必須化が有効だったかを提出ログ単体で復元できる。
  ...
  - strict collect preflight fail-fast 時は `review_command_fail_reason=collect_preflight_check_failed_before_review_command (...)` / `review_command_fail_reason_codes=review_command_collect_preflight_<reason>_before_review_command` / `review_command_fail_reason_codes_source=fallback` / `review_command_retry_command=...` を追加出力し、review quartet の追跡キー欠落を防ぐ。
  - reason code 生成は `c_team_build_collect_preflight_review_reason_code` に統一し、readiness/staging で同じ fail-fast code を出力する。
  ...
  - 同失敗時は `missing_log_review_command=rg -n 'collect_preflight_log_resolved|collect_preflight_log_missing|collect_preflight_check_reason|submission_readiness_retry_command|review_command_fail_reason|review_command_fail_reason_codes|review_command_fail_reason_codes_source|review_command_retry_command' <entry_out>` を出力し、確認コマンドを復旧ログから直接再利用できる。
  ...
  - `collect_c_team_session_evidence.sh --collect-preflight-log <path>` でも、`C_REQUIRE_REVIEW_COMMANDS=1` かつ strict latest（`--collect-latest-require-found 1`）時は同 review command に `--require-review-keys` を付与する。
  - `C_REQUIRE_REVIEW_COMMANDS=1` かつ strict latest（`--collect-latest-require-found 1`）では、上記 `collect_report_review_command` に `--require-review-keys` が付与される。
  - `collect_report_review_command` は `team_status` / `collect_log` パスを shell-safe (`%q`) で埋め込むため、空白を含む一時パスでも再実行コマンドが崩れない。
