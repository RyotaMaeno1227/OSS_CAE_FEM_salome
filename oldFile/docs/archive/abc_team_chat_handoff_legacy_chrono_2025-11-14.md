...
- **Run ID／Evidence**: `docs/archive/legacy_chrono/a_team_handoff.md` と `docs/archive/legacy_chrono/coupled_island_migration_plan.md` に同じ Run ID を記載し、Artifacts は `docs/logs/` / `docs/reports/` / `data/diagnostics/` へ格納する。
...
- **主要ドキュメント**:
  - `docs/archive/legacy_chrono/a_team_handoff.md`: テスト手順・連絡フロー・Evidence テンプレを集約。
  - `docs/archive/legacy_chrono/coupled_island_migration_plan.md`: KPI スナップショットと週次バックログ。
  - `docs/archive/legacy_chrono/chrono_3d_abstraction_note.md`: 3D 抽象化テンプレと進捗バー。
...
- **Pending (2025-11-14)**:
  - 6876543210 系 Run ID を再実行し、Chrono-main との差分を `docs/archive/legacy_chrono/coupled_island_migration_plan.md` の最新 KPI に反映。
...
- **主要ドキュメント**:
  - `docs/archive/legacy_chrono/pm_status_2024-11-08.md`: Bチームの役割と CSV 保守ルール。
...
```
- `python tools/update_descriptor_run_id.py --run-id <ID>` で `docs/logs/kkt_descriptor_poc_e2e.md` と `docs/archive/legacy_chrono/coupled_island_migration_plan.md` を同期。
...
```
- `docs/archive/legacy_chrono/coupled_island_migration_plan.md` の chrono-main 行と同期すること。
...
3. chrono-main から取得した最新 KKT スペクトルログを `tools/compare_kkt_logs.py` で解析し、Δκ・Δpivot を `docs/archive/legacy_chrono/coupled_island_migration_plan.md` に反映。  
...
10. `docs/archive/legacy_chrono/a_team_handoff.md` の「連絡とエスカレーション」節を最新運用（Slack 任意）に合わせて再確認。  
11. KPI 表 (`docs/archive/legacy_chrono/coupled_island_migration_plan.md` §5.1) の進捗値を週次 Run ID に合わせて更新。  
12. Rank 欠落時のトラブルシュート手順を `docs/archive/legacy_chrono/coupled_island_migration_plan.md` のバックログメモへ追記。  
...
1. ✅ 2025-11-16: `workflow_dispatch` で `coupled_endurance.yml` を起動し、`Run #19381264567`（Artifact `coupled-endurance-19381264567`）を `docs/archive/legacy_chrono/pm_status_2024-11-08.md` / 本ドキュメントに記録。  
...
5. ✅ `docs/reports/coupled_endurance_failure_history.md` はアーカイブ扱いを維持（未変更である旨を `docs/archive/legacy_chrono/pm_status_2024-11-08.md` に記載）。  
6. ✅ Aチームと列構成を確認し、合意内容を `docs/archive/legacy_chrono/pm_status_2024-11-08.md` の B ログに追記。  
7. ✅ `docs/documentation_changelog.md` へ今回の CSV/JSON 更新と担当（Bチーム）を登録。  
8. ✅ 監視コマンド `watch -n 60 'tail -n 20 data/coupled_constraint_endurance.csv'` を B ピンへ共有し、本ドキュメントと `docs/archive/legacy_chrono/pm_status_2024-11-08.md` にリンク。  
9. ✅ 失敗 Run の報告テンプレと条件数／Rank 欠落フォーマットを `docs/templates/b_team_endurance_templates.md` に整備。  
10. ✅ `docs/git_setup.md` に Bチーム向け Git 操作チートシートを追記（`git add data/coupled_constraint_endurance.csv data/latest.endurance.json docs/archive/legacy_chrono/pm_status_2024-11-08.md` 等）。  
11. ✅ 複数 Run ID の優先順位ルール（最新成功 > 最新失敗 > 旧成功）を `docs/archive/legacy_chrono/pm_status_2024-11-08.md` に文章化し、本セクションから参照。  
12. ✅ `python scripts/check_preset_links.py` の結果「Preset links verified for 2 file(s).」をチャットに共有し、ログを `docs/archive/legacy_chrono/pm_status_2024-11-08.md` に記録。  
13. ✅ `docs/templates/b_team_endurance_templates.md` に `data/latest.endurance.json` 共有テンプレを追加（外部配布時にコピー運用）。  
14. ✅ 本 B セクションを月次レビュー対象とする旨を `docs/archive/legacy_chrono/pm_status_2024-11-08.md` / `docs/documentation_changelog.md` に記載。  
15. ✅ `workflow_dispatch` 権限・トークン確認手順（Settings > Actions > General のチェックリスト）を `docs/archive/legacy_chrono/pm_status_2024-11-08.md` へ追記し、リーダー共有ログを残す。

**Run 優先順位ルール例**:  
- 同日に 3 つ取得した場合: `最新成功 (#1) > 最新失敗 (#2) > 旧成功 (#3)` の順で `docs/archive/legacy_chrono/pm_status_2024-11-08.md` に記載し、`#2`/`#3` はチャット補足に降格。失敗 Run が唯一の最新動作であれば、暫定主記録として残し、次成功時に差し替える。
...
11. `docs/archive/legacy_chrono/chrono_3d_abstraction_note.md` の学習者向けサマリを抽出し、Learning Path にリンク。（Owner: Cチーム, 期限: 2025-11-18）  
12. `docs/archive/legacy_chrono/pm_status_2024-11-08.md` の「C – Tutorials / Docs」欄に最新進捗コメントを追記。（Owner: Cチーム, 期限: 2025-11-18）  
