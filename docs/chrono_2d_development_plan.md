...
- 今後のタスク候補:
  - `ChronoConstraint2DBase_C` と `ChronoContactPair2D_C` を併せて扱う共通アイランド構造（例: `ChronoIsland2D_C`）を定義。
  - アイランドごとのソルバ呼び出しラッパを実装し、OpenMP などで並列化するパイプラインを試作。
  - テスト: 拘束＋接触が混在する小さなセットアップを用いてアイランドごとにソルバを回し、結果が従来と一致するか、並列時に競合が発生しないかを検証。
- 連続接触シミュレーションテスト (`test_circle_collision_continuous`) を追加し、接触マネージャを用いた複数フレームの滑り/反発処理を確認。
