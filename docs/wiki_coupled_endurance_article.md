...
- 運用担当は `docs/wiki_samples/coupled_endurance_article_sample.md` をコピーし、日付・担当者・リンクを差し替えて利用してください。
- テンプレートに含まれる `{info}` マクロやコードブロックは Confluence 想定です。Notion 等へ移植する場合は同等のインフォボックスを使用します。
- スクリーンショットのファイル名は表 `7.1` と一致させ、Wiki でも同名で添付すると差分管理が容易になります。
...
6. 公開後 24 時間以内にスクリーンショット一覧（表 7.1）が最新リビジョンを指しているか再確認し、必要なら差し替えを行う。
