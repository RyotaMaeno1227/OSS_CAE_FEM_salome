...
## 15分自走拡張指示（5分で終わらない量）
- Aチーム: `@A-team 実行: A5,A8,A12,A14,A17,A18`  
  A5 外部化タスク票化（優先順と移行手順まで）→ A8 警告ログ収集＋修正候補3件 → A12 warn-only→fail 2周＋head/summary 取得 → A14/A17 設定案 → A18 複合拘束ケースの検証観点メモまでを一気に実施。  
- Bチーム: `@B-team 実行: B1,B3,B6,B15`  
  移植前捌きの棚卸し（対象ファイル/差分ポイント）を作成し、Aチーム向けサンプル入出力を整備。報告テンプレは最終形にし、Run ID/Artifact/Log 欄を埋めるガイドを追記。  
- Cチーム: `@C-team 実行: C3,C4,C6,C9,C12,C18`  
  条件数/ピボット解説を1節更新、CSVスキーマ差分チェック、リンク/Lint実行、命名/表記ガイド短文化、用語揺れガイドの追加まで連続実行。  
- 報告: 15分以上の実作業を前提に、`docs/team_status.md` に実施内容と結果を必ず追記。
...
- 報告: 各チームとも Run ID/生成物有無/`git status`/リンクチェック結果を `docs/team_status.md` に追記。
...
## Bチームタスク（移植補助・Aチームの前捌き）
1. Aチーム向けに移植対象ファイルの棚卸し（C→C++ 対応表の下書き）を作成。  
2. 既存C側のAPI境界/入出力（構造体・関数・I/O）を整理し、移植の差分ポイントをメモ化。  
3. 代表ケースの入出力サンプルを整備し、Aチームが即テストできる形で渡す。  
4. 変更がある場合のみ `docs/team_status.md` と `docs/documentation_changelog.md` に記録する。  

※Bチームは「移植作業の前捌き（調査・整理・サンプル整備）」を担当し、CI/運用強化は行わない。
