...
|-----------|------------|----------|-----------------|------------------|--------------|------------|-----------------|------|
| 拘束ディスクリプタ統合 | 設計草稿済み | ████░░░░░ (40%) | 15 | 6 | 3 | 2D ベースの KKT ブロック試作完了 | KKT バックエンドとバッチ API の接合 | `chrono_constraint2d` → 共通 API の PoC |
| 接触ヤコビアン拡張 | 要件定義中 | ██░░░░░░░ (20%) | 18 | 4 | 4 | Rolling/Torsional 係数の分析メモ作成 | 3 点マニフォールド試験実装 | 3D 仕様書との整合確認が必要 |
| 並列バッチスケジューラ | 初期実装中 | ███░░░░░░ (30%) | 20 | 6 | 3 | OpenMP island ランナーに計測フック追加 | TBB ベースのタスク実験 | Island ワークスペースの ABI 固定 |
| Coupled 診断整備 | 設計着手 | ██░░░░░░░ (25%) | 12 | 3 | 2 | 条件数ログの共通化案を整理 | 3D Coupled サンプルで試行 | 2D/3D 共通ダイアグノスティクス構造を調整 |
