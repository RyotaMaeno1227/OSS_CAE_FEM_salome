...
1. **Coupled Solver KKT / Spectral Diagnostics**  
   ...
   - `./chrono-C-all/tests/test_coupled_constraint --use-kkt-descriptor --descriptor-log …` を CI で毎回叩き、`docs/logs/kkt_descriptor_poc_e2e.md` と `docs/reports/kkt_spectral_weekly.md` に evidence を積み上げる（`tools/compare_kkt_logs.py` で差分可視化）。
...
4. **Scheduler / workspace PoC**  
   - `docs/island_scheduler_poc.md` に TBB stub のログ・ベンチ結果・3D workspace 呼び出し例を追記し、`chrono_island2d_workspace_get_*` を使った wrapper の PoC を別ブランチで進める（CI の `bench_island_solver --scheduler` で回す）。
