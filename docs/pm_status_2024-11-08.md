...
|------|-----------|-----------|--------------------|
| **A – Coupled / Contact / Parallel** | KKT 共有化、Iterative Solver、Island 並列 | 進行中: Descriptor PoC / Jacobian 統合 / Iterative Solver。Pending: oneTBB 実測（`bench_island_solver --scheduler tbb`）・Jacobian アーティファクト更新（`tools/run_contact_jacobian_check.py --output-dir ...`）など環境依存タスク。`docs/coupled_island_migration_plan.md` §6 と `docs/a_team_handoff.md` の Pending セクションで同期。 | chrono-main の KKT/Spectral 診断ログを週次で比較し、ディスクリプタ PoC → バッチ統合 → E2E テストのブロッカー（Math ヘルパ、OpenMP→TBB）を洗い出す。|
